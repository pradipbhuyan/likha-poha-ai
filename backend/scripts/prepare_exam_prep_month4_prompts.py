#!/usr/bin/env python3
"""
Month 4 of the 8-month Exam Prep Center question-bank release plan.

Target: 2.5x session-depth floor across every subject. First month where
the gap isn't 100% CUET UG — NEET Chemistry, TOEFL Integrated Skills,
IELTS Vocabulary & Grammar, and SAT Reading & Writing all dipped below
2.5x too (4-12 questions each), substantial enough to include rather than
defer like the 1-2 question exceptions in earlier months.

This generalizes prepare_exam_prep_month{1,2,3}_prompts.py, which
hardcoded exam_type="cuet_ug" throughout, in two ways:

1. Topic schema differs by exam. CUET/NEET topics have an explicit
   "chapter" key distinct from "name" and a numeric "weightage_pct" for
   proportional allocation. TOEFL/IELTS/SAT topics have neither — only
   "name" and "ncert_chapter" (used as the chapter) with no weighting, so
   allocation falls back to equal split. See _topic_chapter/_topic_weight.

2. Grade is no longer always "Grade 12" for a whole subject. NEET
   Chemistry's syllabus spans both NCERT classes (Chemical Bonding and
   Equilibrium are Class 11; the other 5 topics here are Class 12), so
   grade is now tracked per-topic in the manifest (topic_to_grade),
   matching how chapter is already tracked per-topic (topic_to_chapter).

Usage:
    cd backend
    ./venv/bin/python scripts/prepare_exam_prep_month4_prompts.py

Then: run each prompt through GPT-5.5, save as <slug>_questions.json in the
output folder, and ingest with ingest_exam_prep_month4_output.py (dry-run first).
"""
...
OUT_DIR = Path.home() / "Downloads" / "GPT55_ExamPrep_Month4_Prompts"
...
# NEET Chemistry topics whose NCERT chapter is Class 11 (default is Grade 12
# for everything else in this batch — every other target below is uniformly
# Grade 12 already, both in existing DB content and in EXAM_SUBJECTS_MAP).
NEET_CHEMISTRY_GRADE11_TOPICS = {"Chemical Bonding", "Equilibrium"}

# (exam_type, subject, question_count) — gap to reach 2.5x target_per_session,
# recomputed live at prompt-generation time (see module docstring).
MONTH4_TARGETS: list[tuple[str, str, int]] = [
    ("cuet_ug", "Sociology", 21),
    ("cuet_ug", "English", 20),
    ("cuet_ug", "Hindi", 20),
    ...
    ("neet_ug", "Chemistry", 12),
    ("toefl_ibt", "Integrated Skills", 10),
    ("ielts", "Vocabulary & Grammar", 8),
    ("sat", "Reading & Writing", 4),
]
...
def _topic_grade(exam_type: str, subject: str, t: dict) -> str:
    if exam_type == "neet_ug" and subject == "Chemistry" and t["name"] in NEET_CHEMISTRY_GRADE11_TOPICS:
        return "Grade 11"
    return "Grade 12"
...
def _allocate(total: int, weights: list[float]) -> list[int]:
    ...
    topics (e.g. SAT Reading & Writing this month: 4 questions across 6
    topics), only the highest-weighted `total` topics get 1 question each
    and the rest get 0 — forcing a minimum-1-per-topic floor here would
    overshoot the total, unlike Months 1-3 where every subject's gap was
    ...
    if total >= n:
        ...
        while diff < 0:
            ...
            diff += 1
        return base
    # total < n: give 1 to the top `total` topics by weight, 0 to the rest.
...
def main():
    ...
    index_lines = [
        "GPT-5.5 Exam Prep Center authoring — Month 4 (2.5x session-depth floor)",
        "=" * 78,
        "",
        "First month to include exams beyond CUET UG (NEET Chemistry, TOEFL",
        "Integrated Skills, IELTS Vocabulary & Grammar, SAT Reading & Writing).",
        "",
        ...
        "",
        "     cd backend",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month4_output.py --dir {OUT_DIR} --dry-run",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month4_output.py --dir {OUT_DIR}",
        "",
        "Subjects:",
    ]

    manifest = []
    for i, (exam_type, subject, total) in enumerate(MONTH4_TARGETS, start=1):
        ...
        manifest.append({
            ...
            "topic_to_grade": {t["name"]: _topic_grade(exam_type, subject, t) for t, _ in included},
    ...
    total_q = sum(t[2] for t in MONTH4_TARGETS)
    print(f"\nDone. {len(MONTH4_TARGETS)} prompts written to {OUT_DIR} ({total_q} questions total once authored).")
