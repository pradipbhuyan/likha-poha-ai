#!/usr/bin/env python3
"""
Month 7 of the 8-month Exam Prep Center question-bank release plan.

Target: 4.0x session-depth floor across every subject. Sized against
published+draft (non-archived) rows, same rationale as Month 6: Month 6's
508 questions are still status=draft at the time this script was
generated, and sizing against published-only would regenerate content
Month 6 already produced.

No subject clears the floor for the first time this month (JEE Main and
TOEFL Reading/Listening remain comfortably above every floor so far) —
every target below is a subject that's had at least one prior month's
batch already.

CUET UG Mathematics (Domain) needs 21 rather than 20 like every other CUET
subject, for the same reason Sociology needed 21 in Month 6: Month 6's
Mathematics (Domain) batch had one genuine near-duplicate auto-rejected
(20 imported, not 21 — see ingest_exam_prep_month6_output.py's manual
false-positive review), so it's one question short of the other 15
subjects' 140-question non-archived total.

Usage:
    cd backend
    ./venv/bin/python scripts/prepare_exam_prep_month7_prompts.py

Then: run each prompt through GPT-5.5, save as <slug>_questions.json in the
output folder, and ingest with ingest_exam_prep_month7_output.py (dry-run first).
"""
...
OUT_DIR = Path.home() / "Downloads" / "GPT55_ExamPrep_Month7_Prompts"
...
def main():
    ...
    index_lines = [
        "GPT-5.5 Exam Prep Center authoring — Month 7 (4.0x session-depth floor)",
        "=" * 78,
        "",
        "16 CUET UG subjects, all three NEET UG subjects, both SAT subjects,",
        "all three IELTS subjects, and TOEFL Integrated Skills. No subject",
        "clears the floor for the first time this month.",
        "",
        ...
        "",
        "     cd backend",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month7_output.py --dir {OUT_DIR} --dry-run",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month7_output.py --dir {OUT_DIR}",
        "",
        "Subjects:",
    ]

    manifest = []
    for i, (exam_type, subject, total) in enumerate(MONTH7_TARGETS, start=1):
    ...
    total_q = sum(t[2] for t in MONTH7_TARGETS)
    print(f"\nDone. {len(MONTH7_TARGETS)} prompts written to {OUT_DIR} ({total_q} questions total once authored).")
