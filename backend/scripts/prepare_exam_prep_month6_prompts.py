#!/usr/bin/env python3
"""
Month 6 of the 8-month Exam Prep Center question-bank release plan.

Target: 3.5x session-depth floor across every subject. Sized against
published+draft (non-archived) rows, not published-only: Month 5's 443
questions are still sitting as status=draft (not yet published by the
admin) at the time this script was generated, and sizing against
published-only would regenerate content Month 5 already produced. See
_existing_question_stems below, which scans the same non-archived set for
duplicate avoidance — the sizing and the duplicate-avoidance list are now
consistent with each other.

Two subjects clear this floor for the first time: SAT Mathematics (was
exactly at the Month 5 floor, 147/147, now needs 24 more) and IELTS
Reading (deferred at Month 5 with only a 1-question gap — now genuinely
due for volume at 14, same "leftover becomes due" pattern as Month 2's
CUET Biology (Domain)/History/Legal Studies, deferred at Month 1).

CUET UG Sociology needs 21 rather than 20 like every other CUET subject:
Month 5's Sociology batch had one exact-duplicate rejected on import (19
imported, not 20), so it's one question short of the other 15 subjects'
120-question non-archived total.

Usage:
    cd backend
    ./venv/bin/python scripts/prepare_exam_prep_month6_prompts.py

Then: run each prompt through GPT-5.5, save as <slug>_questions.json in the
output folder, and ingest with ingest_exam_prep_month6_output.py (dry-run first).
"""
...
OUT_DIR = Path.home() / "Downloads" / "GPT55_ExamPrep_Month6_Prompts"
...
# (exam_type, subject, question_count) — gap to reach 3.5x target_per_session
# (vs published+draft), recomputed live at prompt-generation time (see
# module docstring).
MONTH6_TARGETS: list[tuple[str, str, int]] = [
    ("cuet_ug", "English", 20),
    ("cuet_ug", "Hindi", 20),
...
def main():
    ...
    index_lines = [
        "GPT-5.5 Exam Prep Center authoring — Month 6 (3.5x session-depth floor)",
        "=" * 78,
        "",
        "16 CUET UG subjects plus all three NEET UG subjects, both SAT",
        "subjects (Mathematics due for the first time), and all three IELTS",
        "subjects (Reading due for the first time) plus TOEFL Integrated",
        "Skills.",
        "",
        ...
        "",
        "     cd backend",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month6_output.py --dir {OUT_DIR} --dry-run",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month6_output.py --dir {OUT_DIR}",
        "",
        "Subjects:",
    ]

    manifest = []
    for i, (exam_type, subject, total) in enumerate(MONTH6_TARGETS, start=1):
    ...
    total_q = sum(t[2] for t in MONTH6_TARGETS)
    print(f"\nDone. {len(MONTH6_TARGETS)} prompts written to {OUT_DIR} ({total_q} questions total once authored).")
