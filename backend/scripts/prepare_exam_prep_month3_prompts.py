#!/usr/bin/env python3
"""
Month 3 of the 8-month Exam Prep Center question-bank release plan.

Target: 2.0x session-depth floor across every subject. Recomputed live
against the current database — Month 2 brought every CUET subject to
exactly 1.5x, so this month every CUET subject (now including Political
Science, which just dipped below the new floor) needs another 20 (13 for
Political Science, whose target_per_session is lower; 21 for Chemistry
(Domain), which sits at 59 not 60 after one exact duplicate was correctly
skipped in Month 2). No other exam needs meaningful work this month except
a 2-question gap in TOEFL Integrated Skills — deferred, same as the Month 1
sub-5-question exceptions; not worth a dedicated round-trip.

Same pattern as Months 1-2: one prompt per SUBJECT, per-topic question-count
table (weightage-proportional, largest-remainder rounding), full existing-
question list embedded as a "do not repeat" block, and an explicit reminder
that the explanation's conclusion must name the same letter as
correct_option (added in Month 2 after Month 1's batch had several
answer-mismatch false-positive flags — Month 2 had zero, so keeping it).

Usage:
    cd backend
    ./venv/bin/python scripts/prepare_exam_prep_month3_prompts.py

Then: run each prompt through GPT-5.5, save as <slug>_questions.json in the
output folder, and ingest with ingest_exam_prep_month3_output.py (dry-run first).
"""
...
OUT_DIR = Path.home() / "Downloads" / "GPT55_ExamPrep_Month3_Prompts"

# (subject, question_count, grade) — gap to reach 2.0x target_per_session,
# recomputed live at prompt-generation time (see module docstring).
MONTH3_TARGETS: list[tuple[str, int, str]] = [
    ("Chemistry (Domain)", 21, "Grade 12"),
    ("English", 20, "Grade 12"),
    ("Hindi", 20, "Grade 12"),
    ("General Test", 20, "Grade 12"),
    ("Physics (Domain)", 20, "Grade 12"),
    ("Mathematics (Domain)", 20, "Grade 12"),
    ("Biology (Domain)", 20, "Grade 12"),
    ("History", 20, "Grade 12"),
    ("Geography", 20, "Grade 12"),
    ("Economics", 20, "Grade 12"),
    ("Accountancy", 20, "Grade 12"),
    ("Business Studies", 20, "Grade 12"),
    ("Sociology", 20, "Grade 12"),
    ("Psychology", 20, "Grade 12"),
    ("Legal Studies", 20, "Grade 12"),
    ("Political Science", 13, "Grade 12"),
]
...
def main():
    ...
    index_lines = [
        "GPT-5.5 Exam Prep Center authoring — Month 3 (2.0x session-depth floor, all CUET)",
        "=" * 78,
        "",
        ...
        "",
        "     cd backend",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month3_output.py --dir {OUT_DIR} --dry-run",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month3_output.py --dir {OUT_DIR}",
        "",
        "Deferred: TOEFL iBT Integrated Skills needs only +2 for 2.0x — not",
        "worth a dedicated round-trip, fold into whichever later month next",
        "touches TOEFL.",
        "",
        "Subjects:",
    ]
    ...
    manifest = []
    for i, (subject, total, grade) in enumerate(MONTH3_TARGETS, start=1):
    ...
    total_q = sum(t[1] for t in MONTH3_TARGETS)
    print(f"\nDone. {len(MONTH3_TARGETS)} prompts written to {OUT_DIR} ({total_q} questions total once authored).")
