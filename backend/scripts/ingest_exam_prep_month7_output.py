#!/usr/bin/env python3
"""
Ingest Month 7 GPT-5.5 Exam Prep Center Output
=================================================
Paired with scripts/prepare_exam_prep_month7_prompts.py. Identical logic to
...
Usage:
    cd backend
    ./venv/bin/python scripts/ingest_exam_prep_month7_output.py --dir ~/Downloads/GPT55_ExamPrep_Month7_Prompts --dry-run
    ./venv/bin/python scripts/ingest_exam_prep_month7_output.py --dir ~/Downloads/GPT55_ExamPrep_Month7_Prompts
"""
...
_SCRIPT_ADMIN = {"profile": {"id": "script:ingest_exam_prep_month7_output"}}
...
def _load_manifest(folder: Path) -> list[dict]:
    ...
    if not manifest_path.exists():
        print(f"ERROR: {manifest_path} not found — run prepare_exam_prep_month7_prompts.py first.")
        sys.exit(1)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Ingest Month 7 GPT-5.5 exam-prep authoring JSON output")
