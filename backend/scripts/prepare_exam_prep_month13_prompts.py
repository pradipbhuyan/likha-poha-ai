#!/usr/bin/env python3
"""
Month 13 — continuing past the original 8-month plan at the same 0.5x-per-
month cadence.

Target: 7.0x session-depth floor across every subject. Sized against
published+draft (non-archived) rows; at generation time everything is
actually published (0 drafts remain, following the Month 12 bulk-publish).

TOEFL Reading — the only subject in the entire 30-subject bank this
pipeline had never touched — finally shows a gap this month, but only 2
questions. Deferred, same "1-2 question exceptions" precedent as Months
1, 3, and 9 (JEE Physics). Every other subject continues as before.

CUET UG Accountancy needs 21 rather than 20 like every other CUET
subject, for the same reason as prior months' +1 catch-ups: Month 12's
Accountancy batch had one genuine near-duplicate rejected (19 imported,
not 20).

Usage:
    cd backend
    ./venv/bin/python scripts/prepare_exam_prep_month13_prompts.py

Then: run each prompt through GPT-5.5, save as <slug>_questions.json in the
output folder, and ingest with ingest_exam_prep_month13_output.py (dry-run first).
"""
...
OUT_DIR = Path.home() / "Downloads" / "GPT55_ExamPrep_Month13_Prompts"
...
# (exam_type, subject, question_count) — gap to reach 7.0x target_per_session
# (vs published+draft), recomputed live at prompt-generation time (see
# module docstring).
MONTH13_TARGETS: list[tuple[str, str, int]] = [
    ("cuet_ug", "English", 20),
    ("cuet_ug", "Hindi", 20),
...
def main():
    ...
    index_lines = [
        "GPT-5.5 Exam Prep Center authoring — Month 13 (7.0x session-depth floor)",
        "=" * 78,
        "",
        ...
        "",
        "     cd backend",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month13_output.py --dir {OUT_DIR} --dry-run",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month13_output.py --dir {OUT_DIR}",
        "",
        "Subjects:",
    ]

    manifest = []
    for i, (exam_type, subject, total) in enumerate(MONTH13_TARGETS, start=1):
    ...
    index_lines.append("Deferred (needs only 2 questions, not worth a dedicated round-trip): "
                        "TOEFL Reading (gap: 2 questions at 7.0x floor)")
    ...
    total_q = sum(t[2] for t in MONTH13_TARGETS)
    print(f"\nDone. {len(MONTH13_TARGETS)} prompts written to {OUT_DIR} ({total_q} questions total once authored).")
