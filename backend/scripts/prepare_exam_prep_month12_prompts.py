#!/usr/bin/env python3
"""
Month 12 — continuing past the original 8-month plan at the same 0.5x-per-
month cadence. One year of monthly authoring rounds.

Target: 6.5x session-depth floor across every subject. Sized against
published+draft (non-archived) rows; at generation time everything is
actually published (0 drafts remain, following the Month 11 bulk-publish).

TOEFL Listening clears the floor for the first time this month (a
4-question gap) — never touched by this pipeline before (was always
comfortably ahead, 6.2-6.9x through Month 11). Included rather than
deferred: 4 questions is the same "substantial enough" size as Month 4's
first SAT Reading & Writing gap, well above the 1-2 question exceptions
this pipeline defers. TOEFL Reading remains the only subject in the
entire bank still untouched (still above 6.5x on its own).

Usage:
    cd backend
    ./venv/bin/python scripts/prepare_exam_prep_month12_prompts.py

Then: run each prompt through GPT-5.5, save as <slug>_questions.json in the
output folder, and ingest with ingest_exam_prep_month12_output.py (dry-run first).
"""
...
OUT_DIR = Path.home() / "Downloads" / "GPT55_ExamPrep_Month12_Prompts"
...
# (exam_type, subject, question_count) — gap to reach 6.5x target_per_session
# (vs published+draft), recomputed live at prompt-generation time (see
# module docstring).
MONTH12_TARGETS: list[tuple[str, str, int]] = [
    ("cuet_ug", "English", 20),
    ("cuet_ug", "Hindi", 20),
...
def _allocate(total: int, weights: list[float]) -> list[int]:
    ...
    and the rest get 0 — triggered this month for TOEFL Listening (4
    questions across 4 topics, so every topic gets exactly 1).
    """
    n = len(weights)
    if total <= 0:
        return [0] * n
...
def main():
    ...
    index_lines = [
        "GPT-5.5 Exam Prep Center authoring — Month 12 (6.5x session-depth floor)",
        "=" * 78,
        "",
        "16 CUET UG subjects, all three NEET UG subjects, all three JEE Main",
        "subjects, both SAT subjects, all three IELTS subjects, and TOEFL",
        "Listening (first time) + Integrated Skills. TOEFL Reading remains",
        "the only untouched subject in the entire bank.",
        "",
        ...
        "",
        "     cd backend",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month12_output.py --dir {OUT_DIR} --dry-run",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month12_output.py --dir {OUT_DIR}",
        "",
        "Subjects:",
    ]

    manifest = []
    for i, (exam_type, subject, total) in enumerate(MONTH12_TARGETS, start=1):
    ...
    total_q = sum(t[2] for t in MONTH12_TARGETS)
    print(f"\nDone. {len(MONTH12_TARGETS)} prompts written to {OUT_DIR} ({total_q} questions total once authored).")
