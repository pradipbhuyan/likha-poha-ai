#!/usr/bin/env python3
"""
Month 11 — continuing past the original 8-month plan at the same 0.5x-per-
month cadence.

Target: 6.0x session-depth floor across every subject. Sized against
published+draft (non-archived) rows; at generation time everything is
actually published (0 drafts remain, following the Month 10 bulk-publish).

No subject clears the floor for the first time this month — every exam
(CUET UG, NEET UG, JEE Main, SAT, IELTS, TOEFL Integrated Skills) now has
at least one prior month's batch behind it. TOEFL Reading/Listening
remain far above every floor this plan has ever set.

CUET UG Physics (Domain) needs 21 rather than 20 like every other CUET
subject, for the same reason as Month 6/7's +1 catch-ups: Month 10's
Physics (Domain) batch had one exact duplicate skipped on import (19
imported, not 20), so it's one question short of the other 15 subjects'
240-question non-archived total.

Usage:
    cd backend
    ./venv/bin/python scripts/prepare_exam_prep_month11_prompts.py

Then: run each prompt through GPT-5.5, save as <slug>_questions.json in the
output folder, and ingest with ingest_exam_prep_month11_output.py (dry-run first).
"""
...
OUT_DIR = Path.home() / "Downloads" / "GPT55_ExamPrep_Month11_Prompts"
...
# (exam_type, subject, question_count) — gap to reach 6.0x target_per_session
# (vs published+draft), recomputed live at prompt-generation time (see
# module docstring).
MONTH11_TARGETS: list[tuple[str, str, int]] = [
    ("cuet_ug", "English", 20),
    ("cuet_ug", "Hindi", 20),
    ("cuet_ug", "General Test", 20),
    ("cuet_ug", "Physics (Domain)", 21),
    ...
    ("neet_ug", "Biology", 45),
    ("jee_main", "Physics", 13),
    ("jee_main", "Chemistry", 13),
    ("jee_main", "Mathematics", 13),
...
def main():
    ...
    index_lines = [
        "GPT-5.5 Exam Prep Center authoring — Month 11 (6.0x session-depth floor)",
        "=" * 78,
        "",
        ...
        "Integrated Skills. No subject clears the floor for the first time",
        "this month.",
        "",
        ...
        "",
        "     cd backend",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month11_output.py --dir {OUT_DIR} --dry-run",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month11_output.py --dir {OUT_DIR}",
        "",
        "Subjects:",
    ]

    manifest = []
    for i, (exam_type, subject, total) in enumerate(MONTH11_TARGETS, start=1):
    ...
    total_q = sum(t[2] for t in MONTH11_TARGETS)
    print(f"\nDone. {len(MONTH11_TARGETS)} prompts written to {OUT_DIR} ({total_q} questions total once authored).")
