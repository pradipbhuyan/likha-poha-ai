#!/usr/bin/env python3
"""
Month 8 — the final month of the original 8-month Exam Prep Center
question-bank release plan.

Target: 4.5x session-depth floor across every subject. Sized against
published+draft (non-archived) rows, same rationale as Months 6-7: Month
7's 511 questions are still status=draft at the time this script was
generated, and sizing against published-only would regenerate content
Month 7 already produced.

No subject clears the floor for the first time this month — JEE Main and
TOEFL Reading/Listening remain comfortably above every floor this plan
has ever set. Every CUET UG subject is back to an even 20 this month (the
Sociology/Mathematics (Domain) +1 "catch-up" adjustments from Months 6-7
are now absorbed — all 16 subjects sit at exactly the same non-archived
count again).

Usage:
    cd backend
    ./venv/bin/python scripts/prepare_exam_prep_month8_prompts.py

Then: run each prompt through GPT-5.5, save as <slug>_questions.json in the
output folder, and ingest with ingest_exam_prep_month8_output.py (dry-run first).
"""
...
OUT_DIR = Path.home() / "Downloads" / "GPT55_ExamPrep_Month8_Prompts"
...
# (exam_type, subject, question_count) — gap to reach 4.5x target_per_session
# (vs published+draft), recomputed live at prompt-generation time (see
# module docstring).
MONTH8_TARGETS: list[tuple[str, str, int]] = [
    ("cuet_ug", "English", 20),
    ("cuet_ug", "Hindi", 20),
...
def main():
    ...
    index_lines = [
        "GPT-5.5 Exam Prep Center authoring — Month 8 (4.5x session-depth floor, final month)",
        "=" * 78,
        "",
        ...
        "clears the floor for the first time this month — this closes out",
        "the original 8-month release plan.",
        "",
        ...
        "",
        "     cd backend",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month8_output.py --dir {OUT_DIR} --dry-run",
        f"     ./venv/bin/python scripts/ingest_exam_prep_month8_output.py --dir {OUT_DIR}",
        "",
        "Subjects:",
    ]

    manifest = []
    for i, (exam_type, subject, total) in enumerate(MONTH8_TARGETS, start=1):
    ...
    total_q = sum(t[2] for t in MONTH8_TARGETS)
    print(f"\nDone. {len(MONTH8_TARGETS)} prompts written to {OUT_DIR} ({total_q} questions total once authored).")
