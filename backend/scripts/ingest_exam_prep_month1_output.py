#!/usr/bin/env python3
"""
Ingest Month 1 GPT-5.5 Exam Prep Center Output
=================================================
Paired with scripts/prepare_exam_prep_month1_prompts.py.

Unlike ingest_exam_prep_gpt55_output.py (one chapter/topic per whole file),
each Month 1 prompt covers a whole SUBJECT with several topics in one batch,
so each question in the output JSON carries its own "topic" field. This
script maps that topic to its chapter via the manifest's topic_to_chapter
table (trusted, not the LLM's own wording) and rejects any question whose
"topic" isn't one of the subject's declared topics rather than silently
mis-filing it.

exam_type/grade/subject are still taken ONLY from the manifest, same
trust model as ingest_exam_prep_gpt55_output.py. Imports go through the
platform's own admin_import_bulk() path (schema validation, dedup, and the
subject/EXAM_SUBJECTS_MAP mapping-integrity check). Everything lands as
status=draft — a separate publish pass is required.

Usage:
    cd backend
    ./venv/bin/python scripts/ingest_exam_prep_month1_output.py --dir ~/Downloads/GPT55_ExamPrep_Month1_Prompts --dry-run
    ./venv/bin/python scripts/ingest_exam_prep_month1_output.py --dir ~/Downloads/GPT55_ExamPrep_Month1_Prompts
"""
...
_SCRIPT_ADMIN = {"profile": {"id": "script:ingest_exam_prep_month1_output"}}
...
# (MD5 fingerprint). This catches near-duplicates/close paraphrases the
# prompt's "do not repeat" instruction didn't fully prevent — most commonly
# the same word-problem template with different numbers, or a light reword.
# This is a lightweight heuristic backstop, not semantic dedup — the prompt's
# embedded "existing questions" list (prepare_exam_prep_month1_prompts.py)
# is the primary defense, since GPT-5.5 can judge real equivalence in a way
# a text-similarity score can't.
#
# Two tiers, calibrated against a real batch: math/formula-heavy questions
# share a lot of templated phrasing ("The value of ∫... dx is:", "If X, then
# Y equals:") even when testing completely different concepts — manual
# review of a real batch found every 56-81%-scored flag was a false
# positive (e.g. Current Ratio vs Quick Ratio, FTC vs a basic definite
# integral), while a genuine same-template/different-numbers clone scored
# 93%. So only >=REJECT_THRESHOLD is dropped outright; the WARN band is
# imported as draft with a review flag, same as the answer-mismatch check.
...
def _tokens(text: str) -> set[str]:
    ...
    # unicode math notation (∫, subscripts/superscripts, operators). An
    # earlier version stripped everything outside [a-z0-9], which collapsed
    # e.g. "∫₀² (x²+1) dx" and "∫₀¹ x f(x²) dx" down to the same leftover
    # connector words ("if", "then", "equals", "dx") and scored two
    # completely different calculus questions as 100% identical.
    text = re.sub(r"[,.:;!?()]", " ", text.lower())
    return {w for w in text.split() if len(w) > 1}


def _similarity(a: str, b: str) -> float:
    """Max of word-overlap (Jaccard) and character-sequence ratio — catches
    both "same words reordered/reworded" and "same structure, different
    numbers" patterns, which either signal alone misses some of."""
...
def _load_manifest(folder: Path) -> list[dict]:
    ...
    if not manifest_path.exists():
        print(f"ERROR: {manifest_path} not found — run prepare_exam_prep_month1_prompts.py first.")
        sys.exit(1)
...
def build_rows(target: dict, llm_questions: list[dict]) -> tuple[list[dict], list[str], list[str]]:
    ...
    human to sanity-check before publishing (see NEAR_DUPLICATE_* comment).
    """
    rows, rejected, warnings = [], [], []
    topic_to_chapter = target["topic_to_chapter"]

    # Existing bank content for near-duplicate checking, refreshed with each
    # accepted row so two near-identical questions in the SAME LLM batch are
    # also caught, not just repeats of pre-existing content.
    ...
    for q in llm_questions:
        if not isinstance(q, dict) or any(not q.get(f) for f in REQUIRED_LLM_FIELDS):
            ...
            continue
        topic = q["topic"]
        if topic not in topic_to_chapter:
            rejected.append(f"unknown topic {topic!r} (not in {list(topic_to_chapter)}): "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Ingest Month 1 GPT-5.5 exam-prep authoring JSON output")
