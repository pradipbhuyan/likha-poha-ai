...
- **Google OAuth — Supabase uses IMPLICIT flow (not PKCE)**: The Supabase project returns tokens in the URL hash (`likhapoha://#access_token=...`), NOT a PKCE code in the query string. `exchangeCodeForSession` FAILS with "both auth code and code verifier should be non-empty". Use `supabase.auth.setSession({ access_token, refresh_token })` for implicit flow. `handleOAuthSuccess` in `login.tsx` detects both flows: if `#access_token` is in hash → `setSession`; if `?code=` in query → `exchangeCodeForSession`.
- **Google OAuth — expo-secure-store fails on emulator without screen lock**: Android Keystore requires a secure lock screen. Without one, `SecureStore.setItemAsync` fails silently (PKCE code_verifier is lost). Fix: `RobustStorageAdapter` in `supabase.ts` writes to `Map` (memory) first, then tries SecureStore. Reads try SecureStore first, fall back to Map.
- **Google OAuth — wasAuthenticated ref in `_layout.tsx`**: Supabase fires `SIGNED_OUT` then `SIGNED_IN` when a new OAuth session replaces an old one. Without this guard, `SIGNED_OUT` would route the user back to login. `wasAuthenticated.current` is set to `true` when `authState` becomes `ready` or `needs_role`. Once set, `unauthenticated` state never routes to login again (only explicit `signOut()` reloads the app and resets it).
- **Google OAuth — post-exchange routing**: After `setSession` or `exchangeCodeForSession`, `handleOAuthSuccess` calls `checkAuthState(session.access_token)` directly and routes: `needs_role_selection=true` → `/auth/role-select` (new users), `false` → `/(tabs)`. Do NOT rely on `_layout.tsx` `onAuthStateChange` for post-OAuth routing — timing conflicts cause incorrect routing.
- **Google OAuth — force account picker**: Add `queryParams: { prompt: "select_account" }` to `signInWithOAuth` in `auth.ts` to prevent auto-login with the cached Google WebView session on subsequent sign-in taps.
...
- **Personalized greeting for Google OAuth users**: `index.tsx` home screen resolves the student name as: `display_name || username || googleName || "Student"`. `googleName` is extracted from `session.user.user_metadata.full_name || .name` during `loadDashboard`. Google OAuth users may have null `display_name` and `username` in the DB profile — the `user_metadata` always has the Google display name.
