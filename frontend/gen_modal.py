#!/usr/bin/env python3
"""Generates the enhanced ReportIssueModal.jsx"""
import os
 ...
 * Enhanced with rich diagnostics + screenshot attachment.
 */
...
function buildBrowserInfo(context, screenshotDataUrl) {
  ...
  return {
    userAgent: navigator.userAgent?.slice(0, 200), platform: navigator.platform,
      ...
      <div data-testid="report-issue-modal"
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          ...
          <button onClick={handleClose} aria-label="Close"
            style={{ background: "none", border: "none", cursor: "pointer", fontSize: "1.2rem", color: "#94a3b8" }}>X</button>
        </div>

        {success ? (
          <div data-testid="report-issue-success">
            <p style={{ textAlign: "center", fontWeight: 700, marginBottom: 8 }}>Thank you for reporting!</p>
            <p style={{ textAlign: "center", fontSize: ".85rem", color: "#64748b" }}>Our team will review and fix this issue.</p>
...
PART3 = '''
            {(mGrade || mSubject || mChapter) && (
              <div style={{ background: "rgba(99,102,241,.06)", borderRadius: 8, padding: "8px 12px",
                fontSize: ".76rem", color: "#4b5563", marginBottom: 12, border: "1px solid rgba(99,102,241,.15)" }}>
