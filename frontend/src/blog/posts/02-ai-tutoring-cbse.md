...
Most AI tutoring tools use a general-purpose language model that has been trained on internet data. When a student asks "What is photosynthesis?", the AI gives a textbook answer — but not necessarily *your* textbook's answer.
...
3. Then it teaches the content in clear, student-friendly language — based on what's actually in the book

This means:
- The definition the AI gives matches what CBSE expects
- The examples are from the NCERT textbook, not Wikipedia
- The difficulty level matches the CBSE syllabus for that grade

## Step-wise Lessons That Mirror How a Tutor Teaches

When a good tutor teaches a new chapter, they don't dump everything at once. They:
1. Introduce the concept simply
2. Explain the core ideas
...
When a student gets confused, they type their question. The AI searches the NCERT chapter for the relevant explanation and answers it in plain language.

This is different from a Google search (which gives random results) or a YouTube video (which requires watching 10 minutes to find the answer).
...
- Families looking for **affordable alternative to tuition** (₹299/month vs ₹2,000–8,000 for a tutor)

Try it free at [likhapoha.in](https://likhapoha.in).
