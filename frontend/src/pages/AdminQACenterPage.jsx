 ...
 * Admin-only page for running and reviewing Lesson Quality Audits.
 * Polls job status while running. Displays report summary, worst lessons, issues.
 * All API calls go to /api/admin/qa/* — admin role required.
 */
...
// ── Module card for future modules ────────────────────────────────────────────
...
export default function AdminQACenterPage({ user }) {
  ...
  async function handleRunAudit() {
    ...
    try {
      ...
      const r = await authFetch("/api/admin/qa/lesson-quality/run", {
        method: "POST",
      ...
      setRunError("Could not start audit. Please try again.");
      setRunning(false);
    }
  }
  ...
  function handleDownload(fmt) {
    authFetch(`/api/admin/qa/lesson-quality/report?format=${fmt}`)
      .then(text => {
        ...
        a.download = `lesson_quality_report.${fmt}`;
        a.click();
      }).catch(() => {});
  }
    ...
    <div data-testid="admin-qa-center" style={{ maxWidth:1100, margin:"0 auto", padding:"0 0 60px", overflowX:"hidden" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:16, alignItems:"flex-start" }}>
        ...
        <Card testid="run-audit-panel">
          ...
          <div style={{ marginBottom:10 }}>
            ...
            {runForm.mode==="full" && (
              <div style={{ marginTop:6, fontSize:".74rem", color:"#f59e0b", fontStyle:"italic" }}>
                ⚠️ Full audit may take several minutes and processes all lessons.
              </div>
            )}
          </div>

          {/* Grade selector */}
          {["grade","grade_subject"].includes(runForm.mode) || runForm.mode==="grade" ? (
            <div style={{ marginBottom:10 }}>
          ...
          {/* Subject selector */}
          {["subject","grade_subject"].includes(runForm.mode) || runForm.mode==="subject" ? (
            <div style={{ marginBottom:10 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
            ...
            {runForm.use_llm && (
              <span data-testid="llm-warning" style={{ fontSize:".7rem", color:"#f59e0b" }}>
                ⚠️ LLM review may use tokens and incur cost.
              </span>
            )}
          </div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:14 }}>
            ...
            <label htmlFor="use_e2e" style={{ fontSize:".78rem", color:"var(--text-muted,#94a3b8)" }}>
              Browser E2E Journey
            </label>
            <span data-testid="e2e-not-configured" style={{ fontSize:".7rem", color:"#94a3b8", fontStyle:"italic" }}>
              (Not configured yet)
            </span>
          </div>

          <button data-testid="run-audit-btn"
            onClick={handleRunAudit} disabled={running}
      ...
      {!loadingReport && report && (
        <div style={{ marginTop:24 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:16 }}>
            <Card>
              <div style={{ fontWeight:700, fontSize:".82rem", marginBottom:8 }}>Score by Grade</div>
              {Object.entries(report.grade_scores||{}).sort((a,b)=>a[0].localeCompare(b[0])).map(([g,s])=>(
                <div key={g} style={{ display:"flex", justifyContent:"space-between", fontSize:".78rem", padding:"2px 0" }}>
                  <span>{g}</span><span style={{ fontWeight:700, color:s>=90?"#22c55e":s>=70?"#f59e0b":"#ef4444" }}>{s}%</span>
                </div>
              ))}
            </Card>
            <Card>
              <div style={{ fontWeight:700, fontSize:".82rem", marginBottom:8 }}>Score by Subject</div>
              {Object.entries(report.subject_scores||{}).sort((a,b)=>a[0].localeCompare(b[0])).map(([s,sc])=>(
                <div key={s} style={{ display:"flex", justifyContent:"space-between", fontSize:".78rem", padding:"2px 0" }}>
                  <span>{s}</span><span style={{ fontWeight:700, color:sc>=90?"#22c55e":sc>=70?"#f59e0b":"#ef4444" }}>{sc}%</span>
                </div>
              ))}
            </Card>
          </div>
      ...
      {loadingReport && (
        <div style={{ textAlign:"center", padding:40, color:"var(--text-muted,#94a3b8)" }}>Loading...</div>
      )}
